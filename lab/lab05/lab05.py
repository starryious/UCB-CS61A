...
def flatten(s):
    ...
    if not type(s) == list:
        return [s]
    return sum([flatten(x) for x in s], [])  # 嵌套判断是不是列表,若是列表则进入下一层列表判断
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    x_1, y_1 = get_lat(city_a), get_lon(city_a)
    x_2, y_2 = get_lat(city_b), get_lon(city_b)
...
# 元素除了是 0、空、FALSE 外都算 TRUE。 元素除了是 0、空、FALSE 外都算 TRUE。
